...
class RivianLongitudinalState:
  ...
  def __init__(self, CP):
    ...
    self._mads_prev_cruise_enabled = False

  def update_stalk_controls(self, ret: structs.CarState, can_parsers, mads_enabled: bool) -> list:
    """Map Rivian's two upward stalk detents to MADS button events.
    ...
    # frame. Rivian uses this gesture to cancel ACC, while MADS lateral remains.
    acc_active_or_just_cancelled = ret.cruiseState.enabled or self._mads_prev_cruise_enabled
  ...
  def update(self, ret: structs.CarState, can_parsers, mads_enabled: bool) -> None:
    button_events = []

    if self.CP.flags & RivianFlags.LONGITUDINAL_HARNESS:
      button_events.extend(self.update_longitudinal_upgrade(ret, can_parsers))

    button_events.extend(self.update_stalk_controls(ret, can_parsers, mads_enabled))
